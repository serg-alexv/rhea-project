...
**Protocol:** `rhea-leftover-preservation/1.1.0`  
**Genome:** [task-genome.json](task-genome.json)  
**Первый проход:** `2026-09-06-cloud-001`, неполный: WD недоступна из проверенной среды.

## 1. Цель и границы

Расширение 1.1.0: пользователь поручил обновить память/расписание, использовать подключённые Google Drive/iCloud по типу данных и освободить локальное место. Актуальные административные проекции находятся в `stash/memory/`. Их роль и границы описаны в [MEMORY.md](../memory/MEMORY.md), а проверяемое освобождение локальных облачных копий — в [STORAGE.md](../memory/STORAGE.md). Это разрешение на provider-managed local dehydration после подтверждённого сохранения; оно не разрешает уничтожать уникальные исходники, облачные originals или доказательства. Исходные runs 1.0.0 сохраняются без изменений.
