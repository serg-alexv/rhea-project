...
def _get_auth_token():
    """Get auth token. Tries: 1) Application Default Credentials, 2) service account, 3) anonymous auth."""
    if _cached_token["token"] and time.time() < _cached_token["expires"] - 60:
        return _cached_token["token"]

    # Try Application Default Credentials (ADC) / Service Account first
    try:
        import google.auth
        from google.auth.transport.requests import Request

        # Force clear the problematic env var if it exists in this process
        if os.environ.get("GOOGLE_APPLICATION_CREDENTIALS") == "/Users/sa/rh.1/firebase/service-account.json":
            del os.environ["GOOGLE_APPLICATION_CREDENTIALS"]

        # If it's still pointing to the wrong place or not set, force the known good path
        if not os.environ.get("GOOGLE_APPLICATION_CREDENTIALS"):
            os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "/Users/sa/.config/gcloud/application_default_credentials.json"
        ...
        if creds:
            ...
            if expiry:
                if isinstance(expiry, datetime):
                    _cached_token["expires"] = expiry.timestamp()
                else:
                    _cached_token["expires"] = float(expiry)
            else:
                _cached_token["expires"] = time.time() + 3500
            return _cached_token["token"]
    except Exception as e:
        # Fallback to service account file if ADC fails
        try:
            from google.auth.transport.requests import Request
