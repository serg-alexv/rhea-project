...
function CrossNav() {
  ...
  return (
    <div
      className="fixed top-0 left-0 right-0 z-50 flex items-center px-3.5 gap-0"
      style={{
        height: '30px',
    ...
    >
      ...
      {/* System panel link — inactive */}
      <a
        href={`${API_BASE}/app`}
        className="relative flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-mono uppercase tracking-widest text-white/38 transition-colors duration-150 hover:text-white/72 hover:bg-white/5"
      >
        {IS_DEV ? <>SYSTEM&apos;S P&amp;W &rarr;</> : <>CONSOLE &rarr;</>}
      </a>
      ...
      <div className="group relative flex items-center gap-1.5 px-2 py-0.5 rounded text-[9px] font-mono uppercase tracking-widest text-cyan-400 cursor-default">
        ...
        <span className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 bg-black/90 border border-white/10 text-white/60 text-[9px] px-2.5 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-[300]">
          3D Knowledge Topology Explorer
        </span>
      </div>
...
export default function Home() {
  ...
  return (
    <>
    <main className="h-screen w-full bg-[#030303] overflow-hidden relative pb-10">
      {/* ─── Cross-navigation bar ─── */}
      <CrossNav />
