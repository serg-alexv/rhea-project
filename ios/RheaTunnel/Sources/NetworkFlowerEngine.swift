...
/// Rhea NetworkFlower Bypass Engine — packet-level anti-censorship.
///
...
public final class NetworkFlowerEngine {
    ...
    public init(config: Config = Config()) {
        self.config = config
        log.info("Network Flower engine initialized. split=\(config.splitClientHello) disorder=\(config.disorder) fakeTTL=\(config.fakePacketTTL.map(String.init) ?? "off")")
    }
