...
def generate_feed() -> str:
    ...
    lines += [
        ...
        "  CONTEXT_MAP: Volovik-Lehninger-Gamow cross-domain synthesis → docs/CONTEXT_MAP.md",
        "    4 universals: entropy=correlations, boundary=irreversibility, tunneling=exp(-barrier/resource), degeneracy=robustness",
        "    3 quanta: planckon (gravity) ≡ ~P (bioenergetics) ≡ codon (information)",
        "    flow ontology: gradient→flux→constraint replaces spacetime as primitive",
        "    5 open questions incl. Ricci flow↔BH thermo, white hole of biology, Λ↔basal metabolic rate",
        "",
        "## Invariants",
