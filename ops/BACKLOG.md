...
## P0 — Must ship (stability → shipping)

- **RHEA-BRIDGE-001** | Add bridge call ledger (jsonl) | Desk: OPS
  Output: logs/bridge_calls.jsonl schema + write path in rhea_bridge.py
  DoD: every call logs provider/model/tokens/latency/status/error; daily summary script exists.
...
- **RHEA-BRIDGE-002** | Provider health probe | Desk: OPS
  ...
  DoD: one command prints table; categorizes 401/402/404/429/400; links to procedures.
  **Status: ⚠️ PARTIAL** — probe script exists (8.7KB), runs but env loading broken (pyenv hashlib). Procedures doc missing.
...
- **RHEA-OFFICE-001** | Office protocol hardening | Desk: LEAD
  ...
  **Status: ⚠️ PARTIAL** — OFFICE.md exists with protocol. Missing: SLA, promotion rules, canonical truth definition.
...
- **RHEA-CTX-002** | Gems ledger w/ IDs | Desk: LEAD
  ...
  **Status: ⚠️ PARTIAL** — GEMS.md exists with 5 gems + IDs. Missing: promotion rule, used_by field.
...
- **RHEA-INC-001** | Incident template & resurrection protocol | Desk: LEAD
  ...
  **Status: ⚠️ PARTIAL** — INCIDENTS.md exists with 5 incidents. Missing: full template fields, dated IDs, verify/rollback/next_test.
...
- **RHEA-COMM-001** | Repo narrative reboot | Desk: LEAD
  ...
  DoD: short, 2026-credible, no overclaims, clear contribution loops.
  **Status: 🔲 TODO**
