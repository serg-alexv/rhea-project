...
/// Full NetworkFlower configuration and monitoring view.
///
/// Provides granular control over every NetworkFlowerEngine technique:
...
public struct NetworkFlowerView: View {

    // MARK: - Persisted Config

    @AppStorage("net.flower.enabled")              private var enabled = false
    @AppStorage("net.flower.splitClientHello")     private var splitClientHello = true
    @AppStorage("net.flower.splitSegments")        private var splitSegments = 2
    @AppStorage("net.flower.tlsRecordSplit")       private var tlsRecordSplit = false
    @AppStorage("net.flower.hostCaseRandomize")    private var hostCaseRandomize = true
    @AppStorage("net.flower.fakePacketEnabled")    private var fakePacketEnabled = false
    @AppStorage("net.flower.fakePacketTTL")        private var fakePacketTTL = 3
    @AppStorage("net.flower.disorder")             private var disorder = false
    @AppStorage("net.flower.oobInjection")         private var oobInjection = false
    @AppStorage("net.flower.targetDomains")        private var targetDomainsRaw = ""
    ...
    private var techniqueToggles: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            techniqueToggle(
                "Host Case Randomization",
                desc: "hOsT: header defeats regex-based HTTP NetworkFlower",
            ...
            VStack(alignment: .leading, spacing: 6) {
                techniqueToggle(
                    "Fake Packet Injection",
                    desc: "Send fake RST with low TTL to confuse NetworkFlower state machine",
            ...
            techniqueToggle(
                "OOB Injection",
                desc: "TCP urgent byte after first split — NetworkFlower fails to reassemble",
                isOn: $oobInjection,
                color: Color.purple
            )
        }
    ...
    private func applyPreset(_ preset: NetworkFlowerEngine.Config) {
    ...
    /// Build a NetworkFlowerEngine.Config from current @AppStorage values.
    public func buildConfig() -> NetworkFlowerEngine.Config {
        var config = NetworkFlowerEngine.Config()
    ...
    private func startStatsPolling() {
        // In a real integration, this would read from the running NetworkFlowerEngine instance
