...
/// Rhea NetworkFlower Engine — packet-level anti-censorship.
///
/// Techniques (based on ZAPRET/tpws, GoodbyeNetwork flower, ByeNetwork flower):
///
/// 1. **TLS ClientHello splitting** — fragments ClientHello across multiple TCP segments
///    so passive NetworkFlower can't read the SNI field in a single pass.
///    Effectiveness: defeats ~90% of passive NetworkFlower (most ISPs worldwide).
///
/// 2. **TLS record splitting** — splits ClientHello into 2 TLS records within a single
///    TCP segment. Some NetworkFlower parsers can't handle multi-record ClientHello.
///
/// 3. **Host header case randomization** — for HTTP, changes `Host:` to `hOsT:` or similar.
///    Servers accept it (HTTP spec is case-insensitive), NetworkFlower regex patterns fail.
///
/// 4. **Fake packet injection** — sends a fake RST/SYN with TTL low enough to reach the
///    NetworkFlower box but expire before the destination server. NetworkFlower sees the fake and resets
///    its state machine, then the real packet sails through.
///
/// 5. **Segment disorder** — sends TCP segments out of order (2,4,6,1,3,5).
///    NetworkFlower sees garbled stream, server reassembles correctly.
///
/// 6. **OOB (Out-of-Band) injection** — sends a TCP urgent byte after the first split
///    segment. NetworkFlower fails to reassemble, server ignores OOB data.
///
...
///     → NetworkFlowerEngine.process(packet) → transformed packets
///       → write back to network
public final class NetworkFlowerEngine {
    ...
    public struct Config {
        ...
        /// Gentle preset — minimal interference, works against simple passive NetworkFlower
    ...
    private let log = Logger(subsystem: "com.rhea.preview", category: "network-flower")
    ...
    public init(config: Config = Config()) {
        self.config = config
        log.info("NetworkFlower engine initialized. split=\(config.splitClientHello) disorder=\(config.disorder) fakeTTL=\(config.fakePacketTTL.map(String.init) ?? "off")")
    }
    ...
    private func splitTLSClientHello(
        ...
        if config.splitPosition > 0 {
            ...
            // Split right before the SNI extension — NetworkFlower can't read the hostname
        ...
        // NetworkFlower reads the garbage as part of SNI and fails pattern matching.
        ...
        if config.disorder {
            // Send in reverse order — NetworkFlower sees fragment 2 first (no SNI yet)
            result.append(frag2)
            result.append(frag1)
        } else {
            result.append(frag1)
